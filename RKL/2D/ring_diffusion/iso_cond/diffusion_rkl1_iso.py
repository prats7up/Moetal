...
def solvr(Y, tau, nx, ny):
	...
	for k in range(nx):
		for l in range(ny):
			...
			if (l==ny-1):
				lp = 0
			qx[k,l] = -D*(Y[k,l]-Y[km,l])/dx
                        qy[k,l] = -D*(Y[k,l]-Y[k,lm])/dy
        for k in range(nx):
                for l in range(ny):
                        kp = k+1
                        if (k==nx-1):
...
def main(s,nx,ny):
	...
	plt.contourf(X, Y, T)
	plt.colorbar()
	fname = 'T_rkl1_iso_'+str(s)+'_'+str(nx)+'_'+str(ny)+'.pdf'
	plt.savefig(fname, format='pdf')
	fname = open('T_rkl1_iso_'+str(s)+'_'+str(nx)+'_'+str(ny)+'.dat','w')
	for j in range(ny):
		np.savetxt(fname, np.column_stack((X[:,j], Y[:,j], T[:,j], bxc[:,j], byc[:,j])))
	fname.close()
	fname = open('timing_rkl1_iso_new.dat','a')
