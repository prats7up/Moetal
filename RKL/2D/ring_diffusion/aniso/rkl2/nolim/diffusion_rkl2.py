...
def main(s,nx,ny):
	...
	#s = 100; 
	w1 = 4./(s*s+s-2.); dt_rkl = 0.25*(s*s+s-2.)*dt_exp
	tend = 1.0
	...
	while (tcode<tend):
		...
		for j in range(2,s+1):
			...
			muj = (2.*j-1.)*bj/(j*bjm1)
                        ...
                        mujt = muj*w1
                        gt = -(1.-bjm1)*mujt
			Ys[2] = muj*Ys[1] + nuj*Ys[0] + (1.-muj-nuj)*Ys0 + solvr(Ys[1], mujt*dt,nx,ny) + solvr(Ys0, gt*dt,nx,ny)
			Ys[0] = Ys[1]
			Ys[1] = Ys[2]
			bjm2 = bjm1
                        bjm1 = bj
	...
	plt.contourf(X, Y, T)
	plt.colorbar()
	fname = 'T_rkl2_'+str(s)+'_'+str(nx)+'_'+str(ny)+'.pdf'
	plt.savefig(fname, format='pdf')
	fname = open('T_rkl2_'+str(s)+'_'+str(nx)+'_'+str(ny)+'.dat','w')
        for j in range(ny):
		np.savetxt(fname, np.column_stack((X[:,j], Y[:,j], T[:,j], bxc[:,j], byc[:,j])))
	fname.close()
	fname = open('timing_rkl2_nolim.dat','a')
