s#Playing aroun with Diemer-Kravtsov 2014 DM profiles & associated potential
import numpy as np
...
def Phi_NFW(r):
    x=r/R200c; c=R200c/rs
    return -G*M200*np.log(1.+c*x)/(r*(np.log(1.+c)-c/(1.+c))) #+3e16
...
rscale1=1.*R200c; r1=0*R200c; p1=1./5.; rscale2=1000*R200c; r2=1.*R200c; p2=1./5.
#rscale1=10*R200c; r1=10*R200c; p1=1./5.; rscale2=10*R200c; r2=10.*R200c; p2=1./5.
f1 = Phi_NFW(r); f2 = (2.*np.pi*G/3.)*d_rho(R200c*100.)*r*r
#w1 = (0.5*(1.+np.tanh((r1-r)/rscale1)))**p1; 
#w2 = (0.5*(1.+np.tanh((r-r2)/rscale2)))**p2
#w1 = (1./(1.+np.exp(-2.*(r1-r)/rscale1)))**p1; w2 = (1./(1.+np.exp(-2.*(r-r2)/rscale2)))**p2
#w1 = (1./(1.+pade66(-2.*(r1-r)/rscale1)))**p1; w2 = (1./(1.+pade66(-2.*(r-r2)/rscale2)))**p2
w1 = (0.5 + np.arctan((r1-r)/rscale1)/np.pi)**p1; w2 = (0.5 + np.arctan((r-r1)/rscale2)/np.pi)**p2
#w1 = (0.5*(1. + erf((r1-r)/rscale1)))**p1; w2 = (0.5*(1. + erf((r-r1)/rscale2)))**p2
phi_fit = f1*w1+f2*w2
...
#g_fit = w1*G*Menc_NFW(r)/(r*r) + w2*(4.*np.pi*G*d_rho(R200c*100.)*r/3.)
#g_fit = g_fit[:-1]
'''
...
plt.ylabel(r'$g(cm^2s^{-1})$')
'''
plt.subplot(211)
rm1=r[:-1]; rm2=rm1[:-1]
rho_fit = np.diff(rm1*rm1*g_fit)/np.diff(rm1*rm1*rm1/3.)/(4.*np.pi*G)
plt.plot(r/R200c,d_rho(r))
plt.plot(rm1[:-1]/R200c,rho_fit)
plt.grid()
plt.xscale('log'); plt.yscale('log')
