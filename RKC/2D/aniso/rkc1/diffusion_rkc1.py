...
def main(s,nx,ny):
	...
	#s = 100; 
	eps = 0.05; dt_rkc = (1.-2.*eps/3.)*s*s*dt_exp; 
	w0 = 1. + eps/(s*s); der = s*eval_chebyu(s-1, w0); w1 = eval_chebyt(s, w0)/der
	tend = 1.0
	...
	plt.contourf(X, Y, T)
	plt.colorbar()
	fname = 'T_rkc1_'+str(s)+'_'+str(nx)+'_'+str(ny)+'.pdf'
	plt.savefig(fname, format='pdf')
	fname = open('T_rkc1_'+str(s)+'_'+str(nx)+'_'+str(ny)+'.dat','w')
	for j in range(ny):
		np.savetxt(fname, np.column_stack((X[:,j], Y[:,j], T[:,j], bxc[:,j], byc[:,j])))
	fname.close()
	fname = open('timing_rkc1_new.dat','a')
